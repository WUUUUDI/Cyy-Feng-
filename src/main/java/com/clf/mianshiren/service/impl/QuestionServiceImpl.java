...
import com.clf.mianshiren.mapper.QuestionMapper;
...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
    ...
    public Page<Question> searchFromEs(QuestionQueryRequest questionQueryRequest) {
        ...
        if (CollUtil.isNotEmpty(tags)) {
            for (String tag : tags) {
                // 我们 tags 是一个数组，这里用 term（精确查询），只要数组里面有 tag，那么此次就返回 true
        ...
        if (StringUtils.isNotBlank(searchText)) {
            ...
            boolQueryBuilder.minimumShouldMatch(1); // 至少需要匹配上面 should 子句中的一个才可以
        }
...
//        }
    ...
    public Page<Question> listQuestionByPage(QuestionQueryRequest questionQueryRequest, HttpServletRequest request) {
        ...
        QueryWrapper<Question> qw = getQueryWrapper(questionQueryRequest);
